...
export default async function BoxOfficeTicketPrintPage({
  ...
  const { data: ticket } = await supabase
    .from("tickets")
    .select("id, qr_hash, status, checked_in_at, order_id, tier_id, event_id")
    .eq("id", ticketId)
    .maybeSingle();
  if (!ticket) notFound();
    ...
    <div className="min-h-screen bg-white px-8 py-10 text-zinc-900 dark:bg-white dark:text-zinc-900">
      ...
      <div className="mx-auto max-w-md">
        ...
        <div className="mt-6 border-t border-zinc-200 pt-4 text-center text-xs text-zinc-400">
          ...
          <p className="mt-1">For refund policy, visit outsiderr.in/legal/terms</p>
        </div>
      </div>
    </div>
  );
