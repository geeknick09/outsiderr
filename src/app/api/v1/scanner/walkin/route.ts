...
export async function POST(request: Request) {
  return withApi(request, async () => {
    ...
    const rl = rateLimit(`walkin:${getRateLimitIdentifier(h)}`, RATE_LIMITS.BOX_OFFICE_ORDER);
    if (rl.limited) return apiError("Too many orders. Please slow down and try again.", 429);

    const supabase = await createClient();

    const { data: pinCheck, error: pinError } = await supabase.rpc("verify_scanner_pin", {
      p_event_id: eventId,
      p_pin: pin,
    });
    if (pinError || !pinCheck?.[0]?.event_id) {
      return apiError("Invalid or inactive scanner PIN.", 401);
    }
