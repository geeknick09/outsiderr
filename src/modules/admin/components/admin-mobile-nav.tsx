...
export function AdminMobileNav() {
  ...
  return (
    <>
      {/* Hamburger — sits in the mobile admin top bar */}
      <button
        type="button"
        onClick={() => setOpen(true)}
        aria-label="Open admin menu"
        aria-expanded={open}
        className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200 bg-white/70 text-zinc-700 transition-colors hover:bg-zinc-100 dark:border-white/10 dark:bg-white/5 dark:text-zinc-200 dark:hover:bg-white/10"
      >
      ...
      {open && (
        <>
          ...
          {/* Slide-in drawer */}
          <aside className="fixed inset-y-0 left-0 z-50 w-64 overflow-y-auto border-r border-zinc-200 bg-white shadow-xl dark:border-white/10 dark:bg-ink lg:hidden">
