...
export function FeaturedCarousel({ events }: { events: EventSummary[] }) {
  ...
  return (
    <section className="mb-10">
      ...
      {/* touch-action: pan-y lets the browser handle vertical scroll natively while we handle horizontal */}
      <div
        ref={trackRef}
...
function CarouselButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="glass flex h-9 w-9 items-center justify-center rounded-full transition-colors hover:shadow-[0_0_16px_rgba(139,92,246,0.4)]"
    >
      {children}
    </button>
  );
