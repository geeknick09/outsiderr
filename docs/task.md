...
|---|---|---|---|
| R0 | Cleanup `apps/` `packages/` stubs + build logs; write 6 docs; migrate root docs → `docs/reference/`; AGENTS.md→pointer; README; Tailwind content glob + ESLint boundary rule (legacy pattern commented until R8) | In progress | |
| R1 | `modules/shared`: ui/, auth/ (supabase clients+middleware), db/ (database.types), lib/ (constants, types, format, pricing, phases, validation, logger, rate-limit, audit, upload, upi, razorpay*, event-lifecycle, organizer-eligibility, backup, cron), hooks/, data/ (events, orders, organizers, organizer profile parts, profile, platform-settings, notifications, waitlist + `listEventOrders`/`listEventTickets` cut from admin.ts), actions/ (auth, profile, notifications, push) | Not started | Middleware imports `shared/auth/middleware` DIRECTLY (edge can't pull pino/pg). Write `scripts/_rewrite_imports.mjs` codemod; update tests/ imports; update `next/dynamic`/`lazy` string paths. |
