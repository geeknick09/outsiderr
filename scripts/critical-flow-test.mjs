...
async function main() {
  ...
  try {
    ...
    if (publishVisible) {
      ...
      log("Event create → redirect to event page", afterUrl.includes("/organizer/events/"), afterUrl);
      ...
      if (newEvents.length > 0) {
            ...
            if (await approveForm.isVisible().catch(() => false)) {
              ...
              if (qrHash) {
                ...
                if (await manualHashInput.isVisible().catch(() => false)) {
                  ...
                  log("Door scan → duplicate scan shows ALREADY USED", pageText.includes("ALREADY USED") || pageText.includes("already"), pageText.substring(0, 100));
                } else {
        ...
        // Use evaluate to find the correct event card and click its Cancel button
        // Each event card has a link to /events/{eventId} — find the one for our event
        const cancelClicked = await page.evaluate((eventId) => {
          ...
          for (const link of links) {
            ...
            if (card) {
              ...
              // Find the Cancel button (not Feature, not Delete)
        ...
        // Use evaluate to find the correct event card and click its Delete button
    ...
    // ================================================================
    // FLOW H: Admin Approve Hero Boost (use a seed event, before QA event is deleted)
    // ================================================================
    console.log("\n=== FLOW H: Admin Approve Hero Boost ===");

    // Use a seed event (not the QA test event) for the hero boost
    const { rows: seedEventForBoost } = await db("SELECT id, title FROM public.events WHERE status = 'PUBLISHED' AND title NOT LIKE 'QA Test%' LIMIT 1");
    if (seedEventForBoost.length > 0) {
      const boostEventId = seedEventForBoost[0].id;

      // Go to organizer event page and purchase a hero boost
      await goto(page, `${BASE}/organizer/events/${boostEventId}`);
      await page.waitForTimeout(3000);
      ...
      if (featureVisible) {
        ...
        const { rows: pendingBoosts } = await db("SELECT id, status FROM public.hero_boosts WHERE event_id = $1 AND status = 'PENDING' ORDER BY created_at DESC LIMIT 1", [boostEventId]);
      ...
      log("Hero boost → no published seed events", false, "");
    }
    ...
    if (pubEventsForFeature.length > 0) {
      ...
      // Use evaluate to find the correct event card
      const featureClicked = await page.evaluate(({ eventId }) => {
        const links = document.querySelectorAll(`a[href="/events/${eventId}"]`);
        for (const link of links) {
          const card = link.closest('.glass');
          if (card) {
            ...
            for (const btn of buttons) {
              ...
              if (text === 'Feature' || text === 'Unfeature') {
                btn.click();
                return true;
              }
            }
    ...
    if (seedEventForRepublish.length > 0) {
      ...
      if (cancelClicked) {
        ...
        if (afterCancel[0]?.status === "CANCELLED") {
          ...
          const republishClicked = await page.evaluate((eventId) => {
            const links = document.querySelectorAll(`a[href="/events/${eventId}"]`);
            for (const link of links) {
              const card = link.closest('.glass');
              if (card) {
                const buttons = card.querySelectorAll('button');
                for (const btn of buttons) {
                  if (btn.textContent?.trim() === 'Re-publish') {
                    btn.click();
                    return true;
                  }
                }
