...
- [x] **Organizer rejection cap + resubmission logic** — Rejected organizers can resubmit until the configured rejection limit is reached. Approved profiles remain eligible; blocked profiles cannot re-enter organizer access. The app now checks eligibility via `kyc_status` + `rejection_count` rather than treating any organizer row as active organizer access.
- [x] **Admin-configurable organizer rejection limit** — The platform settings panel includes `organizer_rejection_limit` (default 5), letting admins control the maximum number of rejections before a user is permanently blocked from organizer access.
- [x] **Live DB migration note** — The canonical schema and fix-all migration were updated to include the organizer rejection tracking fields and the platform setting seed. Any live Supabase instance must apply the migration to reflect these changes in production.
...
- [x] **Organizer Reapplication / Rejection Guardrail** — Rejected profiles are not treated as active organizers. The app tracks `rejection_count` on `organizers`, includes a configurable `organizer_rejection_limit` platform setting, and blocks organizer access permanently once the cap is reached. Rejected-but-resubmittable users can still re-enter the organizer flow before the cap is hit.
